...
import { IPortfolio, IStockSortKeys, ISortObject } from '../../types/portfolio.types';
...
export const Portfolio = ({ data }: IParams) => {
  ...
  const [previousSort, setPreviousSort] = useState<ISortObject | undefined>(undefined)
  ...
  const clickedHeader = (value: string) => {
    ...
    if (previousSort && previousSort.key === sortKey && previousSort.sortDirection === 'asc') {
  ...
  return (
    <div>
      ...
      {portfolioData.stocks.length && <PortfolioHeader clicked={clickedHeader}/>}
      {portfolioData.stocks.map(stock => <PortfolioRow key={stock.ticker} stock={stock} />)}
    </div>
  )
}